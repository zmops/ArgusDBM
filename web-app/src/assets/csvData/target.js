const data = 'mysql,status,,uptime,mysql.status.uptime,Uptime,运行时长,number,s,,MySQL服务进程自上次重启以来的运行时长。,The amount of time since the last restart of the MySQL server process.,"          {\n'
  ...
  + 'pgsql,resource_config,,shared_buffers,pgsql.resource_config.shared_buffers,Shared Buffers,共享内存缓存,int,B,,*共享内存缓存* 是定义数据库服务器用于共享内存缓冲区的内存量。默认值为128MB。建议调整为RAM的25%，但通常不超过40%。,"Defines the amount of memory the database server uses for shared memory buffers.  Default is 128MB.  Guidance on tuning is 25% of RAM, but generally doesn\'t exceed 40%.",,,https://www.pgsql.org/docs/current/static/runtime-config-resource.html#GUC-SHARED-BUFFERS,\n'
  + 'pgsql,resource_config,,work_mem,pgsql.resource_config.work_mem,Memory Size for each Sort,排序内存,int,B,,*work_mem* 定义写入临时磁盘文件之前分配给内部排序操作和哈希表的内存量。默认值为4MB。,The parameter work_mem defines the amount of memory assigned for internal sort operations and hash tables before writing to temporary disk files.  The default is 4MB.,,,https://www.pgsql.org/docs/current/runtime-config-resource.html#GUC-WORK-MEM,\n'
  + 'pgsql,resource_config,,autovacuum,pgsql.resource_config.autovacuum,Autovacuum,自动清理状态,bool,,,*自动清理状态* 定义是否启用自动清理。通常，解决方案是更频繁地清理，而不是更少地清理。,"Whether autovacuum process is enabled or not.  Generally the solution is to vacuum more often, not less.",,,https://www.pgsql.org/docs/current/routine-vacuuming.html#AUTOVACUUM,\n'
  + 'pgsql,resource_config,,max_connections,pgsql.resource_config.max_connections,Max Connections,最大连接数,int,,,*最大连接数* 是允许的最大客户端连接数。请小心更改此值，因为有一些内存资源是按每个客户端分配的，因此将max_connections设置得更高通常会增加pgsql的总体内存使用量。,"The maximum number of client connections allowed.  Change this value with care as there are some memory resources that are allocated on a per-client basis, so setting max_connections higher will generally increase overall pgsql memory usage.",,,https://www.pgsql.org/docs/current/runtime-config-connection.html#GUC-MAX-CONNECTIONS,\n'
  + 'pgsql,resource_config,,effective_cache_size,pgsql.resource_config.effective_cache_size,Disk Cache Size,磁盘缓存大小,int,B,,*磁盘缓存大小* effective_cache_size变量可调整磁盘缓存所需的RAM数量。通常，添加Linux free+cached 是个好主意。查询计划器使用该值来确定计划是否适合内存，如果定义得太低，可能会导致某些计划拒绝某些索引。,"pgsql\'s effective_cache_size variable tunes how much RAM you expect to be available for disk caching.  Generally adding Linux free+cached will give you a good idea.  This value is used by the query planner whether plans will fit in memory, and when defined too low, can lead to some plans rejecting certain indexes.",,,https://www.pgsql.org/docs/current/runtime-config-query.html#GUC-EFFECTIVE-CACHE-SIZE,\n'
  + 'pgsql,resource_config,,wal_buffers,pgsql.resource_config.wal_buffers,Disk-Page Buffers,磁盘分页缓存大小,int,B,,*磁盘分页缓存大小* wal_buffers定义了用于缓存预写日志项的内存量。通常，该值很小（shared_buffers值的3%），但可能需要针对负载过重的服务器进行修改。,"The setting wal_buffers defines how much memory is used for caching the write-ahead log entries. Generally this value is small (3% of shared_buffers value), but it may need to be modified for heavily loaded servers.",,,"https://www.pgsql.org/docs/current/runtime-config-wal.html#GUC-WAL-BUFFERS\n'
  + '\n'
  + 'https://www.pgsql.org/docs/current/runtime-config-resource.html#GUC-SHARED-BUFFERS",\n'
  + 'pgsql,connection,,active,pgsql.connection.active,Active connections,活动连接数,int,,,,,,,https://www.pgsql.org/docs/current/runtime-config-connection.html#GUC-MAX-CONNECTIONS,\n'
  ...
  + 'pgsql,transaction,,commits,pgsql.transaction.commits,Transactions Commits,事务提交次数,float,次,select sum(xact_commit) from pg_stat_database;,,,,,https://www.pgsql.org/docs/current/tutorial-transactions.html,相关于 pg_stat_database_xact_commit\n'
  + 'pgsql,transaction,,rollbacks,pgsql.transaction.rollbacks,Transactions Rollbacks,事务回滚次数,float,次,select sum(xact_rollback) from pg_stat_database;,,,,,https://www.pgsql.org/docs/current/tutorial-transactions.html,相关于 pg_stat_database_xact_commit\n'
  ...
  + 'idle、fastpath function call、disabled、active。",,,,https://www.pgsql.org/docs/current/tutorial-transactions.html,相关于 pg_stat_activity_max_tx_duration\n'
  + 'pgsql,transaction_state,,duration,pgsql.transaction_state.duration,Transactions Duration,事务耗时,,s,,,,,,https://www.pgsql.org/docs/current/tutorial-transactions.html,相关于 pg_stat_activity_max_tx_duration\n'
  ...
  + 'pgsql,lock,,db_name,pgsql.lock.db_name,Locks DB Name,锁所属库名称,,,,,,,,,\n'
  ...
  + 'pgsql,operation,,read,pgsql.operation.read,Operations Read,Blocks操作读频率,float,,,,,,,,\n'
  + 'pgsql,operation,,write,pgsql.operation.write,Operations Write,Blocks操作写频率,float,,,,,,,,\n'
  + 'pgsql,buffer,,allocated,pgsql.buffer.allocated,Allocated,缓冲区Allocated频率,int,,,,,,,,\n'
  + 'pgsql,buffer,,fsync_calls_by_backend,pgsql.buffer.fsync_calls_by_backend,Fsync calls by a backend,缓冲区Fsync calls by a backend频率,int,,,,,,,,\n'
  + 'pgsql,buffer,,written_directly_by_backend,pgsql.buffer.written_directly_by_backend,Written directly by a backend,缓冲区Written directly by a backend频率,int,,,,,,,,\n'
  + 'pgsql,buffer,,written_by_background_writer,pgsql.buffer.written_by_background_writer,Written by the background writer,缓冲区Written by the background writer频率,int,,,,,,,,\n'
  + 'pgsql,buffer,,written_during_checkpoints,pgsql.buffer.written_during_checkpoints,Written during checkpoints,缓冲区Written during checkpoints频率,int,,,,,,,,\n'
  + 'pgsql,conflicts,,db_name,pgsql.conflicts.db_name,DB Name,库名称,string,,,,,,,,\n'
  + 'pgsql,conflicts,,bufferpin,pgsql.conflicts.bufferpin,Pinned buffers,因固定缓冲区取消查询数,float,,,此库中由于固定缓冲区而取消的查询数。,Number of queries in this database that have been canceled due to pinned buffers.,,,,相关于 pg_stat_database_conflicts_confl_bufferpin\n'
  + 'pgsql,conflicts,,deadlock,pgsql.conflicts.deadlock,Deadlocks,因死锁取消查询数,float,,,此库中由于死锁而取消的查询数。,Number of queries in this database that have been canceled due to deadlocks.,,,,相关于 pg_stat_database_conflicts_confl_bufferpin\n'
  + 'pgsql,conflicts,,lock,pgsql.conflicts.lock,Lock timeouts,因锁超时取消查询数,float,,,此库中由于锁超时而取消的查询数。,Number of queries in this database that have been canceled due to lock timeouts.,,,,相关于 pg_stat_database_conflicts_confl_bufferpin\n'
  + 'pgsql,conflicts,,snapshot,pgsql.conflicts.snapshot,Old snapshots,因旧快照取消查询数,float,,,此库中由于旧快照而取消的查询数。,Number of queries in this database that have been canceled due to old snapshots.,,,,相关于 pg_stat_database_conflicts_confl_bufferpin\n'
  + 'pgsql,conflicts,,tablespace,pgsql.conflicts.tablespace,Dropped tablespaces,因删除表空间取消查询数,float,,,此库中由于删除掉的表空间而取消的查询数。,Number of queries in this database that have been canceled due to dropped tablespaces.,,,,相关于 pg_stat_database_conflicts_confl_bufferpin\n'
  ...
  + 'pgsql,checkpoint,,checkpoint_sync_time,pgsql.checkpoint.checkpoint_sync_time,Files Synchronization to disk,文件同步到磁盘时间,,ms,,,,,,,相关于 pg_stat_bgwriter_checkpoint_sync_time\n'
  + 'pgsql,checkpoint,,checkpoint_write_time,pgsql.checkpoint.checkpoint_write_time,Written Files to disk,文件写到磁盘时间,,ms,,,,,,,相关于 pg_stat_bgwriter_checkpoint_write_time\n'
  + 'pgsql,slow_sql,,sql_text,pgsql.slow_sql.sql_text,SQL Text,慢SQL,string,,,需要提前配置并启用 pg_stat_statements。,,,,https://virtual-dba.com/blog/pgsql-performance-identifying-hot-and-slow-queries/,相关于 pg_stat_statements\n'
  ...
  + 'd m,basic,,max_sessions,dm.basic.max_sessions,,最大会话数,,,,,,,,,\n'
