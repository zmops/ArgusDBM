    ...
    <template v-slot:form>
      ...
      >
        <el-form-item label="监控名称" prop="monitor.name">
          <el-input v-model="formData.monitor.name" placeholder="标识监控的名称,名称需要保证唯一性" />
          <div style="font-size: 12px;color: #757D8F">
            <i class="el-icon-info argus-mr-5" style="color: #5A98EC" />标识监控的名称,名称需要保证唯一性
          </div>
        </el-form-item>
        <el-divider />
        ...
        <el-form-item label="采集间隔(秒)">
          ...
          <div style="font-size: 12px;color: #757D8F">
            <i class="el-icon-info argus-mr-5" style="color: #5A98EC" />监控周期性采集数据间隔时间,单位秒
          </div>
        </el-form-item>
        <el-form-item label="测试连接">
          <el-switch v-model="formData.detected" />
          <div style="font-size: 12px;color: #757D8F">
            <i class="el-icon-info argus-mr-5" style="color: #5A98EC" />新增监控前是否先探测检查监控可用性
          </div>
        </el-form-item>
        ...
        <el-form-item label="描述备注">
          <el-input v-model="formData.monitor.description" type="textarea" :rows="3" placeholder="请输入描述备注" />
          <div style="font-size: 12px;color: #757D8F">
            <i class="el-icon-info argus-mr-5" style="color: #5A98EC" />更多标识和描述此监控的备注信息
          </div>
        </el-form-item>
        <el-form-item>
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
      const savadata = this.formatSaveData()
