      ...
      <template v-slot:form>
        <el-row :gutter="20">
          <el-col :span="11">
            ...
            <el-table
              ref="transferLeftTable"
              size="mini"
              stripe
              border
              height="400px"
              :data="lTableData"
              :header-cell-style="{ background: '#E5E6EB' }"
              :header-cell-class-name="'business-table-header'"
              class="BusinessTable"
              @selection-change="handlelSelectionChange"
            >
    ...
    addItem() {
      this.lmultipleSelection.forEach((item) => {
        ...
        this.lTableData.forEach((data, index) => {
          if (item.id == data.id) { this.lTableData.splice(index, 1) }
        })
      })
    },
    removeItem() {
      this.rmultipleSelection.forEach((item) => {
        ...
        this.rTableData.forEach((data, index) => {
          if (item.id == data.id) { this.rTableData.splice(index, 1) }
        })
      })
    },
    handlelSelectionChange(val) {
      if (val.length > 0) { this.lselectDisable = false } else { this.lselectDisable = true }
      this.lmultipleSelection = val
    },
    handlerSelectionChange(val) {
      if (val.length > 0) { this.rselectDisable = false } else { this.rselectDisable = true }
