---
id: package-deploy  
title: Install argusDBM via Package 
sidebar_label: Install via Package
---
> You can install and run argusDBM on Linux Windows Mac system, and CPU supports X86/ARM64. Due to the installation package itself does not include the JAVA runtime environment, you need to prepare JAVA runtime environment in advance.
...
1. Install JAVA runtime environment-refer to[official website](http://www.oracle.com/technetwork/java/javase/downloads/index.html)    
   ...
   ```
2. Download argusDBM installation package
   Download installation package corresponding to your system environment
   - download from [GITEE Release](https://gitee.com/ zmops/argusDBM/releases) repository 
   - download from [GITHUB Release](https://github.com/ zmops/argusDBM/releases) repository

3. Configure argusDBM's configuration file(optional)      
   Unzip the installation package to the host eg: /opt/argusDBM  
   ``` 
   $ tar zxvf argusDBM-[version number].tar.gz   
   ```
   Modify the configuration file `argusDBM/config/application.yml`
...
5. Deploy/Start
   Execute the startup script startup.sh in the installation directory argusDBM/bin/
   ``` 
   $ ./startup.sh 
   ```
...
> 3：Check whether the running log has errors in `argusDBM/logs/` directory. If you haven't solved the issue, report it to the communication group or community.
